...
module.exports = (shepherd) => {
  ...
  shepherd.get('/coindwalletkeys', (req, res, next) => {
    if (shepherd.checkToken(req.query.token)) {
      ...
      let _walletDatLocation = chain === 'safecoin' || chain === 'SAFE' || chain === 'null' ? `${shepherd.safecoinDir}/wallet.dat` : `${shepherd.safecoinDir}/${chain}/wallet.dat`;
              ...
              if (err) {
                ...
                if (!privateKeys) {
                  ...
                  for (let i = 0; i < privateKeys.length; i++) {
                    ...
                    const keyPair = bitcoinJS.ECPair.fromWIF(wifKey, shepherd.electrumJSNetworks.safecoin);
                    const _keyPair = {
                      priv: keyPair.toWIF(),
